...
class Base_Model extends MY_Model{
	 ...
	 *  设置表名
	 */
	function set_table_name($tablename='',$tablepre='aci_')
	{
	...
	final public function listinfo($where = '',$data='*', $order = '', $page = 1, $pagesize = 20, $key='', $setpages = 10,$urlrule = '',$array = array()) {
		$this->number = $this->count($where);  //累计多少数据
		$page = max(intval($page), 1); // 当前的页面
		$offset = $pagesize*($page-1);  //数据查询的起始位置 ，每页默认显示20条数据
		if($offset>$this->number) //如果起始数，大于总的记录数，重新定义页数，和起始位置
		{
			$page=round($this->number/$pagesize);
			$offset = max($pagesize*($page-1),0);
		}
	
		if($this->number > $pagesize){    //累计数据，单页数据，存在翻页，获取翻页的数据，否则没有翻页
