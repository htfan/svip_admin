...
//初始化模块 配置信息，验证模块是否存在，页面的基本路径，seo， 包含 showmessage ，view ，方法
class MY_Controller extends CI_Controller
{
	private  $is_load_captcha;
	public $aci_config;          
	public $aci_status;       
	public $all_module_menu;        //所有要加载的模块也在这里定义
	protected $page_data = array(   //页面基本数据，在这里初始化
		'module_name' => '',
		'controller_name' => '',
		'method_name' => '',
	);
	...
	function __construct(){
		...
		$this->load->driver('cache',array('adapter'=>'file'));  //加载文件缓存驱动器
		$this->load->helper(array('global','url','string','text','language','auto_codeIgniter_helper','member')); //加载辅助函数
                //赋值页面基本数据
		...
		$this->config->load('aci');  //加载aci后台系统的状态和各个模块信息 aci.php (计划把这块去除掉,不用配置文件，用缓存文件，或者时刻更新这块的内容)
		$this->aci_config = $this->config->item('aci_module');  //赋值到定义的变量,通过这里去判断模块是否合法（模块已舍弃）
            
		$_pageseo = $this->config->item($this->router->class,'seo'); //页面seo优化，有没有对应的模块的seo
		...
		$this->all_module_menu = getcache("cache_module_menu_all");  //获取缓存的模型菜单
         
		$this->load->vars($this->page_data);  //把页面的基本配置，加载为缓存变量，以便渲染到模板
                
		$this->_check_module();  //不需要模块，可以去除这部分，一同舍去的还有 aci.php 的这个模块配置文件
	...
	{
		$_aci_config = NULL;  //检查权限里面是否有该路径的访问权力
                //先调取缓存文件，缓存文件没有就去调取数据库，然后生成缓存文件 （缓存文件按角色类别分）
		foreach($this->aci_config as $k=>$v)   //在配置文件里，找到与当前文件 路径，类，方法，配对的配置（文件在配置里面是否存在）
		{
	...
	protected function showmessage($msg, $url_forward = '', $ms = 50000, $dialog = '', $returnjs = '') {
		if($url_forward==''){
...
//初始化未做任何处理 ，重写view ， 新增_load_submenu  ，reload_all_cache
...
//初始化用户基本信息 验证用户是否登录 权限验证  重写 showmessage view 方法
...
//                dump($this->cache_module_menu_arr);
	...
	protected function nav_menu($parent_id, $with_self = 0, $show_where = 0) {
		...
		$result =  array();
//                dump($this->all_module_menu);
		if($this->all_module_menu)
                    //1是显示后台目录，0是显示前台目录
		foreach($this->all_module_menu as $k=>$v){
			if($show_where==1 && strtolower($v['folder'])!="admin") continue; //不是后台目录的跳过
			if($show_where==0 && strtolower($v['folder'])=="admin") continue;  //如果是后台目录不显示，跳过
        ...
        //当前的位置，小导航用的 这里就是一个递归函数 ，实现无限极分类
...
class Admin_Controller extends Member_Controller{
	...
	protected function showmessage($msg, $url_forward = '', $ms = 10000, $dialog = '', $returnjs = '') {

		if($url_forward=='') $url_forward=$_SERVER['HTTP_REFERER'];
		$datainfo = array("msg"=>$msg,"url_forward"=>$url_forward,"ms"=>$ms,"returnjs"=>$returnjs,"dialog"=>$dialog);
               
		echo $this->load->view('admin/header',NULL,true);
		echo $this->load->view('admin/message',$datainfo,true);
		echo $this->load->view('admin/footer',NULL,true);
	...
	protected function check_member() {
		...
		if(!$this->user_id&&!($this->page_data['folder_name']=='admin'&&$this->page_data['controller_name']=='manage'&&$this->page_data['method_name']=='login'))
		{
			$this->showmessage('请您重新登录',site_url('admin/manage/login'));
			exit(0);
		}
	...
	{
            //指定三个页面不用验证权限
		if($this->page_data['folder_name'] =='admin' && $this->page_data['controller_name'] =='manage' && in_array($this->page_data['method_name'], array('login', 'logout','manage'))) return true;
		//指定的超级管理员可以不用验证权限
                if($this->group_id == SUPERADMIN_GROUP_ID) return true;
		//方法是public 开头的，及控制器为manage 方法为go的不用验证权限
	...
	protected function admin_tpl($view_file,$page_data=false,$cache=false)
	{
		...
		$this->load->view('admin/footer',$page_data);
	}
		...
		foreach($this->cache_module_menu_arr as $k=>$_value)
		{
				...
				strtolower($_value['method'])==strtolower(trim($this->page_data['method_name'])))
			{
				...
				if(!$find_menu && isset($_value['arr_parentid'])){
					...
					$arr_parentid = explode(",",$_value['arr_parentid']);
					if(count($arr_parentid)>=2) {
						$parent_id =$arr_parentid[1]; //第二级菜单id
						$third_menu_id =isset($arr_parentid[3])? $arr_parentid[3]:$menu_id; //第三级菜单
					}else{
						$parent_id =$_value['menu_id'];
					}
