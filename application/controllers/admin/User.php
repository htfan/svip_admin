...
class User extends Admin_Controller {
    ...
    function index($page_no = 1) {
        $page_no = max(intval($page_no), 1);  //接收页数
      
        $rate = round(rand(50, 99)/100,2);
      
        $where_arr = array();   //查询的数组 这里只有关键字
        $orderby = $keyword = "";   //排序和关键字默认为空
        if (isset($_GET['dosubmit'])) { //如果是点击搜索过来的，就加入搜索条件
            $keyword = isset($_GET['keyword']) ? safe_replace(trim($_GET['keyword'])) : '';  //过滤关键词
            $keyword == "" ?: $where_arr[] = "concat(username,fullname,email,mobile) like '%{$keyword}%'";
        }
        $where = implode(" and ", $where_arr); //把数组合并成一个查询的条件字符串
        //获取数据列表 已经自动加载了 member_model 
        $data_list = $this->Member_model->listinfo($where, '*', $orderby, $page_no, $this->Member_model->page_size, '', $this->Member_model->page_size, page_list_url('admin/user/index', true));
        $this->view('index', array('data_list' => $data_list, 'pages' => $this->Member_model->pages, 'keyword' => $keyword, 'require_js' => true));
    }

    //验证用户名是否存在 （直接传入用户名查询 和直接根据传递的post数据查询）
    ...
    function add() {
        ...
        if ($this->input->is_ajax_request()) {
            ...
            if ($username == '')
                exit(json_encode(array('status' => false, 'tips' => '用户名不能为空')));
            ...
            $repassword = isset($_POST["repassword"]) ? trim(safe_replace($_POST["repassword"])) : exit(json_encode(array('status' => false, 'tips' => '密码不能为空')));
            if ($repassword == '')
                exit(json_encode(array('status' => false, 'tips' => '重复密码不能为空')));
            if ($repassword != $password)
                exit(json_encode(array('status' => false, 'tips' => '密码输入不一样')));
            ...
            $this->check_username($username) ? exit(json_encode(array('status' => false, 'tips' => '用户名已经存在'))) : '';
    ...
    function user_window($controlId = '', $page_no = 0) {
        ...
        if (isset($_GET['dosubmit'])) {
            ...
            if ($keyword != "")
                $where_arr[] = "concat(username,fullname,email,mobile) like '%{$keyword}%'";
        }
