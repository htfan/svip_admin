...
class Manage extends Admin_Controller {
    ...
    function cache() {
        ...
        $this->showmessage('全局缓存成功', site_url('admin'));
    }
...
//            $this->output->enable_profiler(TRUE);  //输出调试结果
    }
    ...
    function login() {
        if (isset($_POST['username'])) {
            ...
            $username !== "" ?: exit(json_encode(array('status' => false, 'tips' => ' 用户名不能为空')));
            ...
            if (!$r)
                exit(json_encode(array('status' => false, 'tips' => ' 用户名或密码不正确')));
