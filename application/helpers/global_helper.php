...
if ( ! function_exists('thumb'))
{
	...
	if ( ! function_exists('pages'))
	{
		...
		function pages($num, $curr_page, $perpage = 20, $urlrule = '', $array = array(),$setpages = 10) {
			...
			if($num > $perpage) {  //总数大于每页显示的数
				$page = $setpages+1;
				$offset = ceil($setpages/2-1); //左右偏差
				$pages = ceil($num / $perpage);  //总共有多少页
				
				$from = $curr_page - $offset;  //开始显示的页数
				$to = $curr_page + $offset;  // 结束的页数
				$more = 0;
				if($page >= $pages) {  //如果累计页数 小于11页
					$from = 1;
					$to = $pages-1;
				} else {
					if($from <= 1) {   //当前是第一页，起始是1 结束是10
						$to = $page-1;
						$from = 1;
					}  elseif($to >= $pages) {  //结束页大于总的页数
						$from = $pages-($page-2);  
						$to = $pages-1;  
					}
					$more = 1;
